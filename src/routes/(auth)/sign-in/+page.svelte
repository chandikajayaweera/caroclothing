<script lang="ts">
	import { authClient, googleOAuthEnabled } from '$lib/client/auth';
	import {
	...
	async function handleGoogleSignIn() {
		if (loading || !googleOAuthEnabled) return;
		clearErrorMessage();
		loading = true;
		try {
			const callbackURL =
				...
				<div class="space-y-4">
					{#if googleOAuthEnabled}
						...
						<div class="relative flex items-center py-6">
							...
							<span class="mx-4 font-mono text-[10px] tracking-widest text-ash/30 uppercase"
								>OR</span
							>
							<div class="grow border-t border-charcoal"></div>
						</div>
