					...
					{#if $createProductForm.variants.length > 0}
						<div class="mt-5 grid gap-4">
							{#each sortedVariants as variant, index (variant.clientId)}
								...
								{@const isRemovable =
									$createProductForm.variants.length > 1 &&
									variant.clientId !== 'default-color-card'}
																				...
																				>
																					{#each colors as c (c.id)}
																						{@const isColorUsed = $createProductForm.variants.some(
																							(v, idx) =>
																								idx !== originalIndex && v.colorId === c.id
																						)}
																						<DropdownMenu.Item
												...
												<div class="grid gap-1">
													...
													<div class="mt-1 flex flex-wrap gap-2">
														{#each data.sizeOptions as sizeOpt (sizeOpt.value)}
															{@const isSelected = variant.sizes.includes(
																sizeOpt.value as SizeType
															)}
															<button
																type="button"
																onclick={() =>
