...
export const envSchema = z
	.object({
		...
		PUBLIC_SENTRY_REPLAYS_SESSION_SAMPLE_RATE: z.coerce
			...
			.optional()
			.default(0.05),
		PUBLIC_SENTRY_REPLAYS_ON_ERROR_SAMPLE_RATE: z.coerce
			.number()
			.min(0)
			.max(1)
			.optional()
		...
		// Google OAuth (optional, but must be configured as a pair)
		PUBLIC_GOOGLE_CLIENT_ID: optionalCredential,
		...
		TEXT_LK_TRANSACTIONAL_SENDER_ID: z
			.string()
			.min(1, 'TEXT_LK_TRANSACTIONAL_SENDER_ID is required'),
	...
	.superRefine((env, ctx) => {
		if (Boolean(env.PUBLIC_GOOGLE_CLIENT_ID) === Boolean(env.GOOGLE_CLIENT_SECRET)) return;

		ctx.addIssue({
			code: 'custom',
			path: [env.PUBLIC_GOOGLE_CLIENT_ID ? 'GOOGLE_CLIENT_SECRET' : 'PUBLIC_GOOGLE_CLIENT_ID'],
