...
async function sendClaimedNotification(
	...
	if (notification.type === 'auth_welcome') {
	...
	if (notification.type === 'auth_google_linked') {
		if (notification.channel === 'email') {
	...
	if (notification.type === 'shipping_update') {
		...
		if (notification.channel === 'sms') {
			return toSmsDispatchResult(
				await sendShippingUpdateSms(notification.payload as ShippingUpdateSmsInput)
			);
		}
	...
	if (notification.type === 'payment_update') {
		if (notification.channel === 'sms') {
			return toSmsDispatchResult(
				await sendPaymentUpdateSms(notification.payload as PaymentUpdateSmsInput)
			);
		}
