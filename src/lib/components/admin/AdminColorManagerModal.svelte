...
{#if open}
	<!-- Backdrop -->
	<div
		class="fixed inset-0 z-[100] flex items-center justify-center bg-void/85 p-4 backdrop-blur-xs"
		role="dialog"
		aria-modal="true"
	>
		<!-- Modal Card -->
		<div class="relative w-full max-w-md border border-ash/15 bg-charcoal p-6 shadow-2xl">
			<!-- Close button -->
			<button
				type="button"
				onclick={handleClose}
			...
			<div class="mb-6">
				<h3 class="mb-2 font-sans text-xs font-semibold tracking-wider text-ash/60 uppercase">
			...
			<form onsubmit={handleCreate} class="border-t border-ash/10 pt-4">
				<h3 class="mb-3 font-sans text-xs font-semibold tracking-wider text-ash/60 uppercase">
					Create New Color
				</h3>
				...
				<div class="grid gap-3">
					<label class="grid gap-1">
						...
						<input
							...
							disabled={submitting}
							class="min-h-10 border border-ash/20 bg-void px-3 py-1.5 font-sans text-sm text-bone placeholder-ash/40 transition-colors outline-none focus:border-volt"
						/>
					</label>
