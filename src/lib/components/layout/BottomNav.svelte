...
<nav
	class="md:hidden fixed bottom-0 inset-x-0 z-50 bg-void/80 backdrop-blur-md border-t border-charcoal px-2 pb-safe pt-2 h-[calc(60px+env(safe-area-inset-bottom))]"
>
	<div class="flex items-center justify-around h-full">
		{#each navItems as item}
			<a
				href={item.href}
				class="flex flex-col items-center justify-center gap-1 min-w-[64px] transition-colors relative
          {isActive(item.href) ? 'text-volt' : 'text-ash hover:text-bone'}"
			>
				...
				<span class="font-mono text-[10px] uppercase tracking-wider">{item.label}</span>

				{#if item.label === 'Cart' && $cartCount > 0}
					<span
						class="absolute top-0 right-3 bg-volt text-void font-mono text-[9px] leading-none w-4 h-4 rounded-full flex items-center justify-center"
					>
						{$cartCount}
					</span>
				{/if}
