...
CREATE TABLE `cart` (
	...
	CONSTRAINT "cart_has_one_owner" CHECK(("cart"."user_id" IS NOT NULL AND "cart"."session_token" IS NULL) OR ("cart"."user_id" IS NULL AND "cart"."session_token" IS NOT NULL)),
	CONSTRAINT "cart_expiry_positive" CHECK("cart"."expires_at" IS NULL OR "cart"."expires_at" > 0)
);
--> statement-breakpoint
CREATE UNIQUE INDEX `cart_session_token_unique` ON `cart` (`session_token`);--> statement-breakpoint
CREATE INDEX `cart_user_idx` ON `cart` (`user_id`);--> statement-breakpoint
CREATE INDEX `cart_session_idx` ON `cart` (`session_token`);--> statement-breakpoint
CREATE INDEX `cart_expires_idx` ON `cart` (`expires_at`);--> statement-breakpoint
CREATE TABLE `cart_item` (
	...
	CONSTRAINT "cart_item_quantity_range" CHECK("cart_item"."quantity" BETWEEN 1 AND 10),
	CONSTRAINT "cart_item_unit_price_positive" CHECK("cart_item"."unit_price" > 0)
);
--> statement-breakpoint
CREATE INDEX `cart_item_cart_idx` ON `cart_item` (`cart_id`);--> statement-breakpoint
CREATE INDEX `cart_item_variant_idx` ON `cart_item` (`variant_id`);--> statement-breakpoint
CREATE UNIQUE INDEX `cart_item_cart_variant_idx` ON `cart_item` (`cart_id`,`variant_id`);--> statement-breakpoint
CREATE TABLE `drop` (
	`id` text PRIMARY KEY NOT NULL,
	`slug` text NOT NULL,
	`name` text NOT NULL,
	`tagline` text,
	`description` text,
	`status` text DEFAULT 'teaser' NOT NULL,
	`launch_at` integer,
	`end_at` integer,
	...
	`created_at` integer DEFAULT (cast(unixepoch('subsecond') * 1000 as integer)) NOT NULL,
	`updated_at` integer DEFAULT (cast(unixepoch('subsecond') * 1000 as integer)) NOT NULL,
	CONSTRAINT "drop_sort_nonnegative" CHECK("drop"."sort_order" >= 0),
	CONSTRAINT "drop_end_after_launch" CHECK("drop"."launch_at" IS NULL OR "drop"."end_at" IS NULL OR "drop"."end_at" > "drop"."launch_at")
);
--> statement-breakpoint
CREATE UNIQUE INDEX `drop_slug_unique` ON `drop` (`slug`);--> statement-breakpoint
CREATE INDEX `drop_status_launch_idx` ON `drop` (`status`,`launch_at`);--> statement-breakpoint
CREATE INDEX `drop_sort_idx` ON `drop` (`sort_order`,`created_at`);--> statement-breakpoint
CREATE TABLE `drop_product` (
	`drop_id` text NOT NULL,
	`product_id` text NOT NULL,
	`is_hero` integer DEFAULT false NOT NULL,
	`sort_order` integer DEFAULT 0 NOT NULL,
	FOREIGN KEY (`drop_id`) REFERENCES `drop`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`product_id`) REFERENCES `product`(`id`) ON UPDATE no action ON DELETE cascade,
	CONSTRAINT "drop_product_sort_nonnegative" CHECK("drop_product"."sort_order" >= 0)
);
--> statement-breakpoint
CREATE UNIQUE INDEX `drop_product_unique_idx` ON `drop_product` (`drop_id`,`product_id`);--> statement-breakpoint
CREATE UNIQUE INDEX `drop_product_one_hero_per_drop` ON `drop_product` (`drop_id`) WHERE "drop_product"."is_hero" = 1;--> statement-breakpoint
CREATE INDEX `drop_product_drop_idx` ON `drop_product` (`drop_id`);--> statement-breakpoint
CREATE INDEX `drop_product_product_idx` ON `drop_product` (`product_id`);--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `drop_waitlist_drop_contact_idx` ON `drop_waitlist` (`drop_id`,`contact`);--> statement-breakpoint
CREATE INDEX `drop_waitlist_drop_idx` ON `drop_waitlist` (`drop_id`);--> statement-breakpoint
CREATE INDEX `drop_waitlist_contact_idx` ON `drop_waitlist` (`contact`);--> statement-breakpoint
CREATE INDEX `drop_waitlist_user_idx` ON `drop_waitlist` (`user_id`);--> statement-breakpoint
CREATE INDEX `drop_waitlist_notify_idx` ON `drop_waitlist` (`drop_id`,`notified_at`);--> statement-breakpoint
...
CREATE INDEX `product_active_featured_idx` ON `product` (`is_active`,`is_featured`);--> statement-breakpoint
CREATE INDEX `product_gender_active_idx` ON `product` (`gender`,`is_active`);--> statement-breakpoint
CREATE INDEX `product_new_arrival_idx` ON `product` (`is_new_arrival`,`is_active`);--> statement-breakpoint
CREATE INDEX `product_created_idx` ON `product` (`created_at`);--> statement-breakpoint
CREATE INDEX `product_tier_active_idx` ON `product` (`tier`,`is_active`);--> statement-breakpoint
...
CREATE UNIQUE INDEX `shipping_zone_lookup_idx` ON `shipping_zone` (`shipping_method_id`,`district`);--> statement-breakpoint
